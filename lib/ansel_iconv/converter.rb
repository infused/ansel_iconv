    ...
    def iconv(string)
      ...
      until scanner.eos? do
        ...
        when 0x88..0xC8
          ...
          output << (ansi_to_utf8[hex_key] || ansi_to_utf8['ERR']).force_encoding('UTF-16BE').encode('UTF-8')
        ...
        when 0xE0..0xFB
          [2, 1, 0].each do |n| # try 3 bytes, then 2 bytes, then 1 byte
            ...
            if ansi_to_utf8.has_key?(hex_key)
              output << ansi_to_utf8[hex_key].force_encoding('UTF-16BE').encode('UTF-8')
              n.times {scanner.get_byte}
              break
            end
          end
        else
          output << ansi_to_utf8['ERR'].force_encoding('UTF-16BE').encode('UTF-8')
